...
import { cell, playingField } from "../../models/models";
...
const Play = () => {
  ...
  const firstArrayCells: cell[] = [...Array(state).keys()].map((number) => ({
    index: number,
    row: 0,
    col: 0,
    isBomb: false,
  ...
  const [firstPlayingField, setFirstPlayingField] = useState<cell[] | false>(firstArrayCells);
  ...
  const playngFieldRender = () => {
    const mapRender = (item: cell, index: number) => (
      <button
    ...
    return <div className={cn(styles.inner, objCnConditions)}>{firstPlayingField ? firstPlayingField.map(mapRender) : playingFieldState![0].map(mapRender)}</div>;
  };
