import { cell, cellOpen, playingField } from "../../models/models";
...
const playingFieldFunc = (amountCells: 36 | 64 | 256, firstNumber: number): playingField => {
  ...
  const arrayCells: cell[] = [];

  const getNearbyCells = (cell: cell) => {
    const result: cell[] = [];
    for (let i = -1; i < 2; i++) {
  ...
  for (let i = 0; i < numbCellsToOpenNearby; i++) {
    const cell = randomElemFromArray<cell>(cellsForStart);
    const newCell = randomElemFromArray<cell>(getNearbyCells(cell));
  ...
  const openCell: cellOpen = (cell) => {
    const openNearbyCells = (arrCells: cell[]) => {
